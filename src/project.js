...
function nodesFor(ir, semanticClass) {
  return ir.nodes.filter((node) => node.semanticClass === semanticClass);
}

function evidenceSuffix(node) {
  const first = node.evidence[0];
  return first ? ` — ${node.evidenceState.toLowerCase()} · ${first.path}:${first.line}` : '';
}
...
export function projectHarnessCard(ir) {
  const rows = SEMANTIC_CLASSES.map((semanticClass) => {
    const nodes = nodesFor(ir, semanticClass);
    const value = nodes.length > 0 ? nodes.map((node) => node.label).join('; ') : 'Nicht belegt';
...
export function projectJson(ir) {
