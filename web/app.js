...
function renderAll(scroll=true){$("#result-title").textContent=state.ir?.source?.label||state.repo;renderMeta();renderStory();renderInspect();renderSystem();document.body.classList.add("workspace-active");$("#system-nav-scope").textContent=state.repo||"repository";$("#navigation-trail").textContent="Navigation: "+(state.repo||"repository");$("#result").hidden=false;if(scroll)$("#result").scrollIntoView({behavior:"smooth",block:"start"})}
