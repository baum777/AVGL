...
function applyLocale(){
 ...
 const aliases={heroLine1:"hero1",heroLine2:"hero2",heroCopy:"hero",repoLabel:"repo",repoHelp:"help",analysisEyebrow:"analysis",tabStory:"story",tabFiles:"files",tabInspect:"inspect",tabSystem:"system",inheritance:"inheritance",propagation:"propagation",effective:"effective",filesTitle:"filesTitle",filesIntro:"filesIntro",selectFile:"select",selectFileHint:"selectHint",sourceExcerpt:"excerpt",askAvgl:"ask",assistantTitle:"assistant",assistantNoRepo:"noRepo",assistantPlaceholder:"placeholder",searchFiles:"search"};
...
function renderStory(){
 const host=$("#overview-view");host.replaceChildren();const stack=el("div","story-stack");
...
function lensPanel(hostId,title,copy){
 const h=$(hostId);h.replaceChildren();const head=el("div","derived-lens-head");head.append(el("p","eyebrow",title),el("p","derived-lens-copy",copy));h.append(head);return h
}
function renderInheritance(){
 const h=lensPanel("#inheritance-view",t("inheritance"),t("lensInheritance")),path=state.activeFile;
 const rels=(state.ir?.relations||[]).filter(r=>["EXTENDS","INHERITS","OVERRIDES","SHADOWS","APPLIES_TO"].includes(r.type)||r.inheritanceKind).filter(r=>!path||(r.from?.id===path||r.to?.id===path));
 if(!rels.length){h.append(el("p","relation-empty",path?"No evidence-backed inheritance relation for the selected object.":"No evidence-backed inheritance relation in the current IR."));return}
 const list=el("div","derived-lens-list");rels.slice(0,80).forEach(r=>list.append(relationRow(r,path||r.from?.id)));h.append(list)
}
function renderPropagation(){
 const h=lensPanel("#propagation-view",t("propagation"),t("lensPropagation")),path=state.activeFile;
 if(!path){h.append(el("p","relation-empty","Select an object in Structure to trace propagation."));return}
 const rels=(state.ir?.relations||[]).filter(r=>r.from?.kind==="file"&&r.from.id===path);
 const effects=(state.ir?.effects||[]).filter(e=>e.path===path);
 const grid=el("div","file-relation-grid"),a=el("section","file-relation-panel"),b=el("section","file-relation-panel");
 a.append(el("p","eyebrow","Outgoing relations"));const al=el("div","relation-list");rels.slice(0,80).forEach(r=>al.append(relationRow(r,path)));if(!rels.length)al.append(el("p","relation-empty",t("noRelations")));a.append(al);
 b.append(el("p","eyebrow","Effect candidates"));const bl=el("div","relation-list");effects.slice(0,80).forEach(e=>bl.append(effectRow(e)));if(!effects.length)bl.append(el("p","relation-empty",t("noEffects")));b.append(bl);
 grid.append(a,b);h.append(grid)
}
function renderEffective(){
 const h=lensPanel("#effective-view",t("effective"),t("lensEffective")),records=state.ir?.effectiveStates||[];
 if(!Array.isArray(records)||!records.length){h.append(el("p","relation-empty","No effective-state records are present in the current IR. This lens remains capability-gated."));return}
 const list=el("div","derived-lens-list");records.slice(0,80).forEach(r=>{const card=el("div","relation-row");card.append(el("div","relation-type",r.type||r.id||"effective-state"),el("div","relation-target",r.summary||r.value||JSON.stringify(r)));list.append(card)});h.append(list)
}
...
function lensCapabilities(){
 ...
 return {
  inheritance:relations.some(r=>["EXTENDS","INHERITS","OVERRIDES","SHADOWS","APPLIES_TO"].includes(r.type)||r.inheritanceKind),
  propagation:relations.length>0||effects.length>0||chains.length>0,
  effective:Array.isArray(effective)&&effective.length>0
 }
}
function refreshLensCapabilities(){
 const caps=lensCapabilities();
 document.querySelectorAll(".capability-lens").forEach(b=>{const enabled=Boolean(caps[b.dataset.requires]);b.disabled=!enabled;b.title=enabled?"":"Not evidenced by the current IR";});
 return caps
}
function selectView(v){
 ...
 document.querySelectorAll(".view-tab").forEach(b=>{const on=b.dataset.view===state.lens;b.classList.toggle("active",on);b.setAttribute("aria-selected",String(on))});
 allowed.forEach(x=>$("#"+x+"-view").hidden=x!==state.lens);
...
async function loadFile(path,quiet=false){if(!state.repo)return;state.activeFile=path;renderTree();$("#file-empty").hidden=true;$("#file-detail").hidden=false;$("#structure-empty").hidden=true;$("#structure-object").hidden=false;$("#structure-object-path").textContent=path;$("#context-selection").textContent=path.split("/").pop();$("#navigation-trail").textContent="Navigation: "+state.repo+" → "+path.split("/").join(" → ");$("#file-role").textContent=t("fileLoad");$("#file-path").textContent=path;updateAssistantContext();try{const r=await fetch("/api/file",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({repository:state.repo,path,accessMode:state.accessMode})}),p=await r.json();if(!r.ok)throw Error(p.error||t("fileFail"));showFile(p.file)}catch(e){$("#file-role").textContent=t("fileFail");$("#file-summary").textContent=e.message}}
function showFile(f){$("#file-role").textContent=f.role?.[state.locale]||f.sourceKind;$("#file-path").textContent=f.path;$("#file-summary").textContent=f.summary?.[state.locale]||"";const tags=$("#file-tags");tags.replaceChildren(chip(f.sourceKind));(f.semanticClasses||[]).forEach(c=>tags.append(chip(c,"semantic")));const facts=$("#file-facts");facts.replaceChildren(fact(t("role"),f.role?.[state.locale]||f.sourceKind),fact(t("areas"),(f.semanticClasses||[]).join(" · ")||"—"),fact(t("symbols"),(f.symbols||[]).join(", ")||"—"),fact(t("lines"),String(f.lineCount||"—")),fact(t("size"),fmtBytes(f.size)));renderFileRelations(f.path);const evidenceCount=(state.ir?.nodes||[]).flatMap(n=>n.evidence||[]).filter(e=>e.path===f.path).length;$("#context-evidence").textContent=evidenceCount?evidenceCount+" bounded evidence refs":"No bounded evidence ref on current semantic nodes";$("#file-excerpt").textContent=f.excerpt||"";renderInheritance();renderPropagation();renderEffective();refreshLensCapabilities()}
...
function renderFileRelations(path){
 ...
 $("#context-relations").textContent=visible.length?visible.length+" source-backed relation"+(visible.length===1?"":"s"):"No resolved relation";
 ...
 $("#context-effect").textContent=effects.length?effects.length+" effect candidate"+(effects.length===1?"":"s"):"No resolved effect candidate"
}
